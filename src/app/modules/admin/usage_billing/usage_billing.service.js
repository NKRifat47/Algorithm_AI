...
export const AdminUsageBillingService = {
  getUsageAndBillingData: async (prisma) => {
    ...
    const { _sum: { price: totalRevenue } } = await prisma.subscription.aggregate({ _sum: { price: true } });
    
    // 2. API Requests
    const { _sum: { requestCount: totalRequests, tokensUsed: totalTokens } } = await prisma.apiUsage.aggregate({ 
    ...
    const { _sum: { price: recentRev } } = await prisma.subscription.aggregate({ 
      where: { createdAt: { gte: thirtyDaysAgo } }, 
      _sum: { price: true } 
    });
    const { _sum: { price: prevRev } } = await prisma.subscription.aggregate({ 
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const displayDateStr = d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
      chartDataMap[dateStr] = { displayDate: displayDateStr, fullDate: dateStr, tokens: 0, requests: 0 };
    }
